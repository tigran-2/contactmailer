...
- **Threading/Concurrency**: Concurrent email sending via `ThreadPoolExecutor` in background threads.
...
- **SSE (Server-Sent Events)**: Real-time progress tracking in the browser via a socket-to-SSE bridge.
...
3. **Seed initial data (Superuser & Samples):**
   ...
   ```
4. **View live socket progress (Terminal):**
   ```bash
...
2. **Sample CSVs**: Located in the `samples/` directory:
   ...
   - `contacts_bulk.csv`: 100 contacts for testing pagination and real-time progress.
...
4. **Trigger Campaign**: Create a campaign, choose a target tag (or leave blank), and click **Send Now**.
5. **Real-time Progress**: 
   - **Web UI**: A progress bar will appear on the campaign list page.
   - **Terminal**: You can also track progress by running the `progress_socket` client.
